import * as Inputs from '../../inputs';
import { builder } from '../../../builder';
import { defineQuery, defineQueryFunction, defineQueryObject } from '../../utils';

export const countUserRoleQueryArgs = builder.args((t) => ({
  where: t.field({ type: Inputs.UserRoleWhereInput, required: false }),
  orderBy: t.field({ type: [Inputs.UserRoleOrderByWithRelationInput], required: false }),
  cursor: t.field({ type: Inputs.UserRoleWhereUniqueInput, required: false }),
  take: t.field({ type: 'Int', required: false }),
  skip: t.field({ type: 'Int', required: false }),
  distinct: t.field({ type: [Inputs.UserRoleScalarFieldEnum], required: false }),
}))

export const countUserRoleQueryObject = defineQueryFunction((t) =>
  defineQueryObject({
    type: 'Int',
    nullable: false,
    args: countUserRoleQueryArgs,
    resolve: async (_root, args, _context, _info) =>
      await _context.prisma.userRole.count({
        where: args.where || undefined,
        cursor: args.cursor || undefined,
        take: args.take || undefined,
        skip: args.skip || undefined,
        orderBy: args.orderBy || undefined,
      }),
  }),
);

export const countUserRoleQuery = defineQuery((t) => ({
  countUserRole: t.field(countUserRoleQueryObject(t)),
}));
